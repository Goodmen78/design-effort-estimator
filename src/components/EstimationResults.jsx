...
const FoodyardEstimationResults = () => {
  ...
  const merged = estimationData
    ? { ...estimationData.overview, ...estimationData }
    : null;
  ...
  const getColorScheme = () => {
    ...
    if (projectType.includes("food") || projectType.includes("automotive"))
      return "red";
    if (projectType.includes("tech") || projectType.includes("b2b"))
      return "blue";
    if (projectType.includes("health") || projectType.includes("finance"))
      return "green";
    if (projectType.includes("creative")) return "purple";
    if (projectType.includes("commerce") || projectType.includes("retail"))
      return "orange";
  ...
  if (error) {
    return (
      <div
        className={`min-h-screen bg-gradient-to-br ${colorScheme.primary} p-5`}
      >
        <div className="max-w-7xl mx-auto bg-white bg-opacity-95 backdrop-blur-lg rounded-3xl p-10 shadow-2xl">
          <h1
            className={`text-center text-gray-800 mb-10 text-4xl font-bold bg-gradient-to-r ${colorScheme.primary} bg-clip-text text-transparent`}
          >
            Design Project Plan
          </h1>
  ...
  const totalPersonDays =
    ...
    420;
  const teamMembers =
    merged?.teamMembers || estimationData.teamComposition?.length || 8;
  ...
  const projectIcon = merged?.projectType?.toLowerCase().includes("automotive")
    ? "🚗"
    : merged?.projectType?.toLowerCase().includes("food")
    ? "🍽️"
  ...
  return (
    <div
      className={`min-h-screen bg-gradient-to-br ${colorScheme.primary} p-5 font-sans`}
    >
      <div className="max-w-7xl mx-auto bg-white bg-opacity-95 backdrop-blur-lg rounded-3xl p-10 shadow-2xl">
        <h1
          className={`text-center text-gray-800 mb-10 text-4xl font-bold bg-gradient-to-r ${colorScheme.primary} bg-clip-text text-transparent`}
        ...
        <div className="mb-10 bg-white p-6 rounded-2xl shadow-lg border border-gray-200">
          <h2
            className={`text-gray-700 text-3xl font-semibold mb-5 border-b-4 border-${colorScheme.light} pb-3`}
          >
            📊 Project Overview
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-5 my-5">
            <div
              className={`bg-gradient-to-br ${colorScheme.card} text-white p-5 rounded-2xl text-center shadow-lg`}
            >
            ...
            >
              ...
              <div className="text-sm opacity-90">
                Working Days ({timelineWeeks} weeks)
              </div>
            </div>
            <div
              className={`bg-gradient-to-br ${colorScheme.card} text-white p-5 rounded-2xl text-center shadow-lg`}
            >
              <div className="text-3xl font-bold mb-1">
                {merged?.estimatedFeatures?.length || "12"}
              </div>
          ...
          <div className="bg-green-50 border border-green-500 rounded-lg p-4 my-5 text-green-800">
            ...
            {merged?.projectType || "Multi-platform application"} with
            comprehensive design system, user research, and cross-platform
            consistency.
          </div>
        </div>
          ...
          <div className="mb-10 bg-white p-6 rounded-2xl shadow-lg border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
              {merged.estimatedFeatures.map((feature, index) => (
                <div
                  key={index}
                  className="bg-gray-50 p-4 rounded-lg border border-gray-200 text-sm text-gray-700"
                >
                  ✓ {feature}
                </div>
              ))}
        ...
        {/* Team Composition */}
        {estimationData.teamComposition &&
          estimationData.teamComposition.length > 0 && (
            <div className="mb-10 bg-white p-6 rounded-2xl shadow-lg border border-gray-200">
              <h2
                className={`text-gray-700 text-3xl font-semibold mb-5 border-b-4 border-${colorScheme.light} pb-3`}
              >
        ...
        {/* Screen Breakdown */}
        {estimationData.screenBreakdown &&
          estimationData.screenBreakdown.length > 0 && (
            <div className="mb-10 bg-white p-6 rounded-2xl shadow-lg border border-gray-200">
              <h2
                className={`text-gray-700 text-3xl font-semibold mb-5 border-b-4 border-${colorScheme.light} pb-3`}
              >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
                <div
                  className={`bg-gradient-to-br ${colorScheme.accent} text-white p-5 rounded-2xl text-center shadow-lg`}
                >
                  <div className="text-3xl font-bold mb-1">
                    {estimationData.screenBreakdown.length}
                  </div>
                  <div className="text-sm opacity-90">Unique Screens</div>
                </div>
                <div
                  className={`bg-gradient-to-br ${colorScheme.secondary} text-white p-5 rounded-2xl text-center shadow-lg`}
                >
                  <div className="text-3xl font-bold mb-1">
        ...
        {/* Complexity Summary */}
        {estimationData.complexitySummary &&
                      ...
                      >
                        <td className="p-3 align-top">
                          <span
                            ...
                            style={getComplexityStyles(c.complexity)}
                          >
                            {c.complexity?.toUpperCase()}
                          </span>
                        </td>
                        <td className="p-3 align-top">
                          <strong>{c.uniqueScreens}</strong>
                        </td>
                        <td className="p-3 align-top">
        ...
        {estimationData.roleEffortDistribution &&
          estimationData.roleEffortDistribution.length > 0 && (
            <div className="mb-10 bg-white p-6 rounded-2xl shadow-lg border border-gray-200">
              <h2
                className={`text-gray-700 text-3xl font-semibold mb-5 border-b-4 border-${colorScheme.light} pb-3`}
              >
        ...
        {/* Phase Allocation */}
        {estimationData.phaseAllocation &&
                  ...
                  <tbody>
                    {estimationData.phaseAllocation.map((phase, idx) => (
                      ...
                      >
                        ...
                        <td className="p-3 align-top">
                          <strong>{phase.totalEffort}h</strong>
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              ...
              <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                ...
                <ul className="text-sm text-green-700 space-y-1">
                  {estimationData.resourcePlan.specialistTeam?.map(
                    (member, i) => (
                      <li key={i}>• {member}</li>
                    )
                  )}
              ...
              <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                ...
                <p className="text-sm text-purple-700">
                  {estimationData.resourcePlan.teamScaling}
                </p>
              </div>
            </div>
          </div>
              ...
              >
                <thead>
                  <tr className="bg-slate-700 text-white">
                    <th
                      className="p-3 text-left font-semibold border-r border-slate-600 sticky left-0 bg-slate-700 z-10"
                    ...
                    {ganttData[0]?.weeks.map((week, idx) => (
                      <th
                        key={idx}
                        className="p-3 text-center font-semibold border-r border-slate-600 text-xs"
                        style={{ minWidth: "80px" }}
                      >
                        W{week.week}-1
                      </th>
                      ...
                      {roleData.weeks.map((week, weekIdx) => (
                        <React.Fragment key={weekIdx}>
                          <td className="p-2 text-center border-r border-gray-200">
                            <div
                              className={`${week.bg} text-white px-2 py-1 rounded text-xs font-bold mx-auto`}
                              style={{ maxWidth: "60px" }}
                            >
                              {week.label}
                            </div>
        ...
        {/* Weekly Roadmap */}
        {estimationData.weeklyRoadmap &&
          estimationData.weeklyRoadmap.length > 0 && (
            <div className="mb-10 bg-white p-6 rounded-2xl shadow-lg border border-gray-200">
              <h2
                className={`text-gray-700 text-3xl font-semibold mb-5 border-b-4 border-${colorScheme.light} pb-3`}
              >
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        <div
                          className={`px-3 py-1 bg-gradient-to-r ${colorScheme.primary} text-white text-sm font-semibold rounded-lg`}
                        >
                          Week {week.week}
                        </div>
                        <div className="text-sm font-medium text-gray-600">
                          Phase:{" "}
                          <span className="text-gray-800">{week.phase}</span>
                        </div>
                      </div>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                ...
                <ul className="text-sm text-blue-700 space-y-2">
                  {estimationData.qualityAssurance.reviewPoints?.map(
                    (point, i) => (
              ...
              <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                ...
                <ul className="text-sm text-green-700 space-y-2">
                  {estimationData.qualityAssurance.clientCheckpoints?.map(
                    (checkpoint, i) => (
                      <li key={i} className="flex items-start">
              ...
              <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                ...
                <p className="text-sm text-purple-700">
                  {estimationData.qualityAssurance.testingStrategy}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <ul className="text-sm text-gray-700 space-y-2 bg-red-50 p-4 rounded-lg border border-red-200">
                  {estimationData.riskAssessment.timelineRisks?.map(
                    (risk, i) => (
                      <li key={i}>• {risk}</li>
                    )
                  )}
              ...
              <div>
                ...
                <ul className="text-sm text-gray-700 space-y-2 bg-orange-50 p-4 rounded-lg border border-orange-200">
                  {estimationData.riskAssessment.complexityRisks?.map(
                    (risk, i) => (
                      <li key={i}>• {risk}</li>
                    )
                  )}
              ...
              <div>
                ...
                <ul className="text-sm text-gray-700 space-y-2 bg-yellow-50 p-4 rounded-lg border border-yellow-200">
                  {estimationData.riskAssessment.resourceRisks?.map(
                    (risk, i) => (
                      <li key={i}>• {risk}</li>
                    )
                  )}
            ...
            <div className="grid md:grid-cols-3 gap-6">
              ...
              <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                ...
                <ul className="text-sm text-purple-700 space-y-2">
                  {estimationData.successMetrics.clientSatisfaction?.map(
                    (factor, i) => (
                      <li key={i} className="flex items-start">
        ...
        <div
          className={`bg-gradient-to-br ${colorScheme.primary} p-8 rounded-2xl text-white text-center`}
        >
          <h3 className="text-2xl font-bold mb-4">
            Project Estimation Complete
          </h3>
          <p className="text-lg opacity-90 mb-2">
            Total Effort: <strong>{totalPersonDays} Person-Days</strong> |
            Timeline: <strong>{timelineWeeks} Weeks</strong>
          </p>
          <p className="text-sm opacity-80">
            This comprehensive plan provides a detailed roadmap for successful
            project delivery with quality assurance, risk mitigation, and clear
            success metrics.
          </p>
        </div>
      </div>
